...
public:
    vector<int> shortestAlternatingPaths(int n, vector<vector<int>>& redEdges, vector<vector<int>>& blueEdges) {
      ...
      vector<vector<pair<int, int>>>adj(n);   
        ...
        for (auto &it : redEdges) {          //to traverse red edges.
            adj[it[0]].push_back({it[1], 0});  //Push two things node i.e it[1] and colr red 0;
        }
        for (auto &it : blueEdges) {
            adj[it[0]].push_back({it[1], 1}); // same but blue color 1;  
        }
        vector<int>ans(n, -1);
